...
export class InterventionResponseDto {
  ...
  @ApiPropertyOptional({
    example:
      'Détail: Réparation urgente | Vélo: VTT Giant rouge | Problème: Frein arrière défaillant',
  })
  detail?: string;
...
export class DisponibiliteResponseDto {
  @ApiProperty({
    example: true,
    description: 'Indique si le créneau est disponible',
  })
  disponible: boolean;

  @ApiPropertyOptional({
    type: TechnicienDto,
    description: 'Technicien disponible (si disponible)',
  })
  technicien?: TechnicienDto;
}
